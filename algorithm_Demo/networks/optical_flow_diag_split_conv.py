# optical_flow_diag_split_conv.py
# 方向: 斜向（对角线）
# 变体: split + conv —— layer_1 不再是纯相位选择，而是用带 padding 的 4x4 卷积核
#       [1,2,1] 沿对角线做一次平滑（相当于先低通再抽样），提高信噪比。
...
weights = np.zeros((2, 1, 4, 4), dtype=np.int8)
for i in range(2):
    weights[i, 0, range(i,i+3), range(i+2, i-1, -1)] = [1, 2, 1]
create_layer(
...
weights = np.zeros((2, 1, 4, 4), dtype=np.int8)
for i in range(2):
    weights[i, 0, range(i, i+3), range(1-i, 4-i)] = [1, 2, 1]
create_layer(
...
# ===========================================================================
NETWORK_NAME = "optical_flow_diag_split_conv"
