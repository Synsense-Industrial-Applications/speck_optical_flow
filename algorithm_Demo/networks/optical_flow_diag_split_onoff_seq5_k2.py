# optical_flow_diag_split_onoff_seq5_k2.py
# 方向: 斜向（对角线）
# 变体: split + on/off + seq5 + k2 —— 事件序列长度提到 5（seq of 5）+ 最小 2x2 内核，
#       用于对比更长序列对判向稳定性的影响。
...
# ===========================================================================
NETWORK_NAME = "optical_flow_diag_split_onoff_seq5_k2"
READOUT_LAYER = layer_4          # 变量名，等于 create_layer 的 layer_name
READOUT_LAYER_NAME = "layer_4"
READOUT_SHAPE = (63, 63, 8)      # (size_x, size_y, features)
SLOW_CLOCK = None
DVS_BRANCH_2_ENABLED = False
WARNINGS = ['末层非 1x1 核，跳过补边（补边手法只适用于 1x1 头）', 'config.dvs_layer.destinations[1] 被注释：第二条 split 分支收不到 DVS 输入（Demo_SNN.py 已证实这会让 S-family features 1/2/5/6 不输出）']
