...
weights = np.zeros((8, 1, 2, 2), dtype=np.int8)
for i in range(2):
    for j in range(2):
        weights[i*2+j, 0, i, j] = 1
        weights[4 + i*2 + j, 0, i, j] = 1
create_layer(
...
create_layer(
    ...
    weights=w_1_to_2[::2].astype('int8'),
    # monitor_enable=True,
...
create_layer(
    ...
    output_shape_feature=4,output_shape_size_x=64,output_shape_size_y=64,
    threshold_high=2,threshold_low=-1,
    weights=w_1_to_2[1::2].astype('int8'),
    # monitor_enable=True,
...
weights = np.concatenate([w_2_to_3[::2, ::2], w_0_to_3[::2]], axis=1).astype('int8')
...
weights = np.concatenate([w_2_to_3[1::2, 1::2], w_0_to_3[1::2]], axis=1).astype('int8')
create_layer(
...
weights = np.concatenate([w_3_to_4[::2, ::2], w_2_to_4[::2, ::2]], axis=1).astype('int8')
create_layer(
...
weights = np.concatenate([w_3_to_4[1::2, 1::2], w_2_to_4[1::2, 1::2]], axis=1).astype('int8')
create_layer(
